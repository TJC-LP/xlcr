 ...
 *
 * Besides keeping track of the steps themselves, this object also takes care of boot‑strapping the
 * converter (BridgeRegistry) and splitter (DocumentSplitter) infrastructure that those steps rely
 * on. In particular we:
 *
 *   1. Always call [[BridgeRegistry.init]] so that the core set of converters is available. 2.
 *      Optionally register Aspose‑powered converters and splitters *afterwards* so they override
 *      the core defaults when desired.
 *
 * Aspose integration is enabled when either of the following is set to a value that resolves to
 * logical *true* ("true", "1", or "yes"):
 *
