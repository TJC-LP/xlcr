...
object RegistryDiagnostics {
  ...
  /**
   * Logs comprehensive information about ServiceLoader discovery and registry state. This helps
   * diagnose issues where Aspose providers aren't being found.
   */
  ...
  def getDiagnosticReport(): String = {
    ...
    sb.append(
      s"ServiceLoader found: ${bridgeProviders.size} BridgeProvider(s), ${splitterProviders.size} SplitterProvider(s)\n"
    )
    ...
    sb.append(
      s"Registry contains: ${bridges.size} total bridges, ${asposeBridges.size} Aspose bridges\n"
    )
