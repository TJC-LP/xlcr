...
import com.tjclp.xlcr.compression.{AnchorExtractor, CompressionPipeline}
import com.tjclp.xlcr.compression.models.CellInfo
import com.tjclp.xlcr.compression.utils.SheetGridUtils
...
trait ExcelToLLMParser[I <: MimeType] extends Parser[I, CompressedWorkbook] {
  ...
  private def extractFileName(inputPath: String): String = {
    ...
    Try {
      ...
      .filter(name => name != null && !name.isEmpty) // Ensure filename is not null or empty
      .getOrElse {
        logger.warn(s"Could not extract a valid filename from input path: '$inputPath'. Using default 'excel-file.xlsx'.")
...
class XlsxToLLMParser(
                       ...
                       val excelParser: Parser[MimeType.ApplicationVndOpenXmlFormatsSpreadsheetmlSheet.type, SheetsData]
...
object ExcelToLLMParser {
  ...
  def forXls(config: SpreadsheetLLMConfig = SpreadsheetLLMConfig()): XlsToLLMParser = {
    ...
    logger.warn("Creating XLS parser using SheetsDataExcelParser. Ensure this parser implementation supports HSSF (.xls) format. Casting parser type.")
    ...
    val specificParser = new SheetsDataExcelParser().asInstanceOf[Parser[MimeType.ApplicationVndMsExcel.type, SheetsData]]
