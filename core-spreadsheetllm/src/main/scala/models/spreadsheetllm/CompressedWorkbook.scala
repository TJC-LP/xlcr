...
/**
 * Represents a compressed Excel workbook containing multiple sheets. This is the top-level
 * container for the SpreadsheetLLM compression output.
 *
 * @param fileName
 *   Original file name (without path)
 * @param sheets
 *   List of compressed sheets in the workbook
 * @param metadata
 *   Additional metadata about the workbook and compression
 */
case class CompressedWorkbook(
  ...
  /**
   * Returns all children models (sheets in this case). Required by the Model trait.
   */
   ...
   *
   * @return
   *   A map of statistics about the compression
   */
  def compressionStats: Map[String, Any] = {
    val totalOriginalCells =
      sheets.map(s => s.originalRowCount.toLong * s.originalColumnCount.toLong).sum
    val totalCompressedEntries = sheets.map(_.content.size).sum
    val compressionRatio =
      if (totalCompressedEntries > 0) totalOriginalCells.toDouble / totalCompressedEntries else 1.0
   ...
   *
   * @param sheet
   *   The compressed sheet to add
   * @return
   *   A new CompressedWorkbook with the added sheet
   */
   ...
   * @return
   *   Option containing the found sheet, or None if not found
   */
