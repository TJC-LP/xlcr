...
/**
 * InvertedIndexTranslator converts the pruned spreadsheet grid into a compact dictionary format
 * that maps unique cell content to locations.
 *
 * This is the second step in the SpreadsheetLLM compression pipeline, applied after AnchorExtractor
 * has pruned away less-informative cells.
 */
object InvertedIndexTranslator {
  private val logger = LoggerFactory.getLogger(getClass)

  /**
   * Converts a grid of cells into a dictionary mapping unique content to locations. This inverted
   * index approach dramatically reduces redundancy by grouping cells with the same content and
   * merging them into 2D rectangular ranges.
   *
   * @param grid
   *   The sheet grid to process
   * @param config
   *   Configuration options including coordinate correction settings
   * @return
   *   A map from unique cell content to locations (either a single range or a list of addresses)
   */
  def translate(
    grid: SheetGrid,
      ...
      if (content.isEmpty) {
        ...
        val addresses = cellEntries.map { case ((row, col), cellInfo) =>
          val (finalRow, finalCol) =
            if (
              config.preserveOriginalCoordinates && cellInfo.originalRow.isDefined && cellInfo
                .originalCol.isDefined
            ) {
          ...
          if (config.verbose && cellInfo.originalRow.isDefined) {
            logger.debug(
              s"Cell mapping: internal($row,$col) -> A1:${addr.toA1Notation} [${cellInfo.value}]"
            )
          }
          addr
        }
    ...
    logger.info(
      f"Inverted index: $originalCellCount cells -> $uniqueContentCount entries ($compressionRatio%.2fx compression)"
    )
    result
  }
  ...
  /**
   * Represents a rectangular range of cells in (startRow, startCol) -> (endRow, endCol) form. We
   * can convert it to A1 notation: e.g. "A1:B5".
   */
  ...
  /**
   * Attempts to merge addresses into as few 2D rectangular ranges as possible. 1) We repeatedly
   * pick an unvisited cell, expand the largest rectangle from it, 2) Remove that rectangle's
   * addresses, and repeat. 3) Then we run an optional "coalesceRectangles" pass to unify any
   * adjacent rectangles.
   *
   * @param addresses
   *   Sequence of cell addresses to merge
   * @return
   *   Sequence of A1 notation strings
   */
   ...
   */
  private def isFullRectangle(
    topRow: Int,
    leftCol: Int,
    bottomRow: Int,
    rightCol: Int,
    addrSet: collection.mutable.Set[CellAddress]
  ): Boolean = {
    var r = topRow
    while (r <= bottomRow) {
      var c = leftCol
  ...
  /**
   * Attempt to merge rectangles that share identical row or column boundaries and form a hole-free
   * combined rectangle. This can further reduce the final set of rectangles if the main expansion
   * logic discovered them separately.
   *
   * We'll do a naive repeated approach: for each rectangle, see if we can unify it with another.
   * Stop once no merges occur.
   */
  private def coalesceRectangles(
    rects: List[CellRange],
    allAddrs: Set[CellAddress]
  ): List[CellRange] = {
    var changed = true
    var current = rects
  ...
  /**
   * If r1 and r2 can be merged (i.e. they share identical columns and adjacent rows, or identical
   * rows and adjacent columns), and the union is hole-free, return Some(merged). Otherwise, None.
   */
  ...
  /**
   * Check if every cell in [top..bottom, left..right] is indeed in the set of addresses. This is
   * used by the coalescing function to confirm the union is hole-free.
   */
  private def isFullRect(
    top: Int,
    left: Int,
    bottom: Int,
    right: Int,
  ...
  /**
   * Finds groups of consecutive addresses according to the provided adjacency test. (Currently not
   * used by the new 2D merging logic, but kept in case we need it.)
   */
