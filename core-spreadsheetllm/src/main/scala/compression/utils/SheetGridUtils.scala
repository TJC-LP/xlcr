...
object SheetGridUtils {
   ...
   *
   * @param sheetData
   *   The sheet data from the core Excel model
   * @return
   *   A SheetGrid suitable for anchor extraction
   */
   ...
   *   The sheet grid
   * @param region
   *   The table region to analyze
   * @return
   *   Count of non-empty cells in the region
   */
  ...
  /**
   * Calculates the content diversity in a region. A higher diversity score means a more varied
   * content type mix (text, numbers, dates, etc.)
   *
   * @param grid
   *   The sheet grid
   * @param region
  ...
  def calculateContentDiversity(grid: SheetGrid, region: TableRegion): Double = {
    ...
    val formattedCells =
      cells.count(c => c.hasFillColor || c.isBold || c.hasBottomBorder || c.hasTopBorder)
   ...
   *   The table region to analyze
   * @return
   *   Density as a ratio between 0.0 and 1.0
   */
  ...
  /**
   * Checks if a row is empty (contains only empty cells) For table boundary detection, we need to
   * be strict about emptiness
   */
  ...
  /**
   * Checks if a column is empty (contains only empty cells) For table boundary detection, we need
   * to be strict about emptiness
   */
   ...
   *   The sheet grid
   * @param startRow
   *   The row to start checking from
   * @param direction
   *   Direction to check (1 for down, -1 for up)
   * @param left
   *   Leftmost column of the range to check
   * @param right
   *   Rightmost column of the range to check
   * @param maxRows
   *   Maximum number of rows to check
   * @return
   *   Number of consecutive empty rows
   */
  def countConsecutiveEmptyRows(
    grid: SheetGrid,
    startRow: Int,
    direction: Int,
    left: Int,
   ...
   *   The sheet grid
   * @param startCol
   *   The column to start checking from
   * @param direction
   *   Direction to check (1 for right, -1 for left)
   * @param top
   *   Top row of the range to check
   * @param bottom
   *   Bottom row of the range to check
   * @param maxCols
   *   Maximum number of columns to check
   * @return
   *   Number of consecutive empty columns
   */
  def countConsecutiveEmptyCols(
    grid: SheetGrid,
    startCol: Int,
    direction: Int,
    top: Int,
  ...
  /**
   * Checks if a row could be considered an anchor row based on its content and formatting An anchor
   * row has significant content, formatting, or structural characteristics
   */
  ...
  /**
   * Checks if a column could be considered an anchor column based on its content and formatting An
   * anchor column has significant content, formatting, or structural characteristics
   */
  ...
  /**
   * Checks if a region has uniform content (all non-empty cells have the same value) This is useful
   * for identifying test/example tables or grids with repeating values.
   *
   * @param grid
   *   The sheet grid
   * @param region
   *   The table region to analyze
   * @return
   *   True if all non-empty cells have the same value
   */
