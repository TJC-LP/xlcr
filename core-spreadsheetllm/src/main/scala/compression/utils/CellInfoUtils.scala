...
object CellInfoUtils {
   ...
   *
   * @param cellData
   *   The CellData from the core Excel model
   * @return
   *   CellInfo suitable for anchor extraction
   */
  def fromCellData(cellData: CellData): CellInfo = {
    ...
    if (colIndex < 0) {
      logger.warn(
        s"Detected negative column index: $colIndex in cell ${cellData.referenceA1}, corrected to 0"
      )
    }
   ...
   * @return
   *   true if the content appears to be filler
   */
   ...
   *   The string to check
   * @return
   *   true if the string is a repeated pattern
   */
  ...
  /**
   * Helper method to determine the formatting pattern in a row or column. This helps detect
   * structure based on borders, colors, and formatting.
   */
