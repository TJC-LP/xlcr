...
/**
 * Implementation of the TableSense algorithm for table detection. This is a more sophisticated
 * multi-pass approach for smaller sheets that uses gap analysis, boundary lines, and multiple
 * criteria for high quality detection.
 */
object TableSenseDetector {
  ...
  def detect(
    ...
    logger.info(
      s"Found ${nonEmptyRows.size} non-empty rows and ${nonEmptyCols.size} non-empty columns"
    )
    ...
    logger.info(s"Non-empty rows: ${if (sortedRows.size > 20) s"${sortedRows.size} rows"
      else sortedRows.mkString(", ")}")
    logger.info(s"Non-empty columns: ${if (sortedCols.size > 20) s"${sortedCols.size} columns"
      else sortedCols.mkString(", ")}")
    logger.info(
      s"Row gaps (${rowGaps.size}): ${rowGaps.map(g => s"${g._1}-${g._2}").mkString(", ")}"
    )
    logger.info(
      s"Column gaps (${colGaps.size}): ${colGaps.map(g => s"${g._1}-${g._2}").mkString(", ")}"
    )
    ...
    expectedTableRegions.foreach { case (rowStart, rowEnd, colStart, colEnd) =>
      logger.info(
        f"  - Table from row $rowStart to $rowEnd, column $colStart to $colEnd (${rowEnd - rowStart + 1}x${colEnd - colStart + 1})"
      )
    }
    ...
    if (config.verbose) {
      ...
      contentRegions.foreach { region =>
        logger.info(f"Content region at (${region.topRow},${region.leftCol}) to (${region
    ...
    val contentRegionsWithAnchors = contentRegions.map { region =>
      ...
      logger.info(f"Adding content region as table: (${region.topRow},${region.leftCol})-(${region
          .bottomRow},${region.rightCol}): ${region.width}x${region.height}")
    ...
    val filteredCandidates = uniqueCandidates.filter { region =>
      ...
      val widthToHeightRatio =
        if (region.height > 0) region.width.toDouble / region.height else Double.MaxValue
      ...
      if (config.verbose) {
        ...
        logger.debug(
          f"Table candidate at (${region.topRow},${region.leftCol}) to (${region.bottomRow},${region.rightCol}): " +
      ...
      if (config.verbose) {
        logger.debug(
          f"  - Final decision: big enough=$isBigEnough, content dense=$isContentDense, " +
    ...
    if (config.enableCohesionDetection && cohesionRegions.nonEmpty) {
      ...
      enhancedRegions =
        CohesionDetector.applyOverlapCohesionFilter(enhancedRegions, cohesionRegions)
      // Apply border cohesion filter
      enhancedRegions =
        CohesionDetector.applyOverlapBorderCohesionFilter(enhancedRegions, cohesionRegions)
    }
    ...
    finalRegions.zipWithIndex.foreach { case (region, index) =>
      logger.info(f"Table ${index + 1}: (${region.topRow},${region.leftCol})-(${region
          .bottomRow},${region.rightCol}), ${region.width}x${region.height}")
    }
  ...
  /**
   * Directly detects distinct content regions in the grid, useful for spreadsheets with clearly
   * separated content blocks. This approach works better than gap-based detection for test cases
   * with "islands" of content.
   */
  private def detectDistinctContentRegions(
    grid: SheetGrid,
    ...
    for {
      ...
      if (checkCellContent(grid, r, c) && !visited(r)(c)) {
        ...
        while (queue.nonEmpty) {
          ...
          for ((dr, dc) <- directions) {
            ...
            if (gapTooLarge && logger.isDebugEnabled) {
              logger.debug(
                f"Gap detected between ($currentRow,$currentCol) and ($newRow,$newCol) - treating as separate regions"
              )
            }
        ...
        logger.info(
          f"Found content island at ($topRow,$leftCol) to ($bottomRow,$rightCol): ${width}x${height}, cells: ${regionCells
              .size}, density: ${density}%.2f"
        )
    ...
    if (regions.nonEmpty) {
      ...
      logger.info(
        "No connected regions found, but non-empty cells exist. Analyzing cell distribution..."
      )
      ...
      for ((rowStart, rowEnd) <- rowRanges)
        for ((colStart, colEnd) <- colRanges) {
          ...
          if (regionCells.nonEmpty) {
            ...
            logger.info(
              f"Found cell cluster at ($rowStart,$colStart)-($rowEnd,$colEnd): cells=${regionCells.size}"
            )
            ...
            logger.info(
              f"  → Refined to ($actualRowStart,$actualColStart)-($actualRowEnd,$actualColEnd)"
            )

            // Create the table region
            clusterRegions += TableRegion(
              actualRowStart,
              actualRowEnd,
              actualColStart,
              actualColEnd,
      ...
      for (rowGroup <- rowGroups)
        for (colGroup <- colGroups) {
          ...
          if (regionCells.size >= 2) { // At least 2 cells to form a table
            ...
            logger.info(
              f"Created table from cell group at ($minRow,$minCol)-($maxRow,$maxCol): cells=${regionCells.size}"
            )
            lastResortRegions += TableRegion(
              minRow,
              maxRow,
              minCol,
              maxCol,
  ...
  /**
   * Helper method to find consecutive ranges in a sequence of indices For example, [1,2,3,7,8,12]
   * becomes [(1,3), (7,8), (12,12)]
   */
  ...
  /**
   * Helper function to convert row and column gaps into expected table regions This is used for
   * debugging to understand what tables should be created
   */
  ...
  /**
   * Checks if a cell has content at the specified coordinates To reliably detect all content, we
   * use isEmpty instead of isEffectivelyEmpty
   */
  ...
  private def filterLittleBoxes(regions: List[TableRegion], grid: SheetGrid): List[TableRegion] =
    ...
    regions.filter { region =>
      ...
      if (cellCount > 0) {
        logger.info(
          f"Keeping table region (${region.topRow},${region.leftCol})-(${region.bottomRow},${region.rightCol}) with $cellCount cells"
        )
        logger.info(f"  → Sample content: $contentSamples")
        true
      } else {
        logger.info(f"Filtering out empty table region (${region.topRow},${region
            .leftCol})-(${region.bottomRow},${region.rightCol})")
        false
      }
    }
