...
object TableDetector {
  ...
  /**
   * Refine boundaries of regions using enhanced trimming provided by RegionGrowthDetector.
   * This delegates to the potentially more sophisticated logic there.
   */
    ...
    if (topRow <= bottomRow) {
        ...
        if (topRow != region.topRow || bottomRow != region.bottomRow) {
             ...
             TableRegion(topRow, bottomRow, region.leftCol, region.rightCol,
                 updatedAnchorRows, region.anchorCols)
        } else {
             region // No change
        }
    } else {
        // Region became invalid after trimming, return an invalid region or handle appropriately
        TableRegion(region.topRow, region.topRow - 1, region.leftCol, region.leftCol - 1, Set.empty, Set.empty) // Example invalid
    }
  }
  ...
  private def trimEmptySides(region: TableRegion, grid: SheetGrid): TableRegion = {
    ...
    if (region.topRow > region.bottomRow) return region // Skip if already invalid from top/bottom trim
     ...
     if (leftCol <= rightCol) {
        ...
        if (leftCol != region.leftCol || rightCol != region.rightCol) {
            ...
            TableRegion(region.topRow, region.bottomRow, leftCol, rightCol,
               region.anchorRows, updatedAnchorCols)
        } else {
            region // No change
        }
    } else {
