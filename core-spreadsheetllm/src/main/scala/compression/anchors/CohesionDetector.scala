...
object CohesionDetector {
   ...
   * @return
   *   List of detected cohesion regions
   */
  def detectCohesionRegions(
    grid: SheetGrid,
    ...
    val allCohesionRegions =
      (formattingBasedRegions ++ contentBasedRegions ++ mergedCellRegions ++ formulaRegions)
        .distinct
   ...
   */
  private def createCohesionRegionFromCells(
    cells: Set[(Int, Int)],
    cohesionType: CohesionType
  ): CohesionRegion = {
    if (cells.isEmpty) {
      throw new IllegalArgumentException(
        "Cannot create a cohesion region from an empty set of cells"
      )
    }
  ...
  /**
   * Filter that removes table candidates overlapping with cohesion regions unless they fully
   * contain or are fully contained by the cohesion region.
   */
  def applyOverlapCohesionFilter(
    tableRegions: List[TableRegion],
    cohesionRegions: List[CohesionRegion]
  ): List[TableRegion] = {
   ...
   */
  def applyOverlapBorderCohesionFilter(
    tableRegions: List[TableRegion],
    cohesionRegions: List[CohesionRegion]
  ): List[TableRegion] = {
   ...
   */
  def applyFormulaCorrelationFilter(
    grid: SheetGrid,
    tableRegions: List[TableRegion]
  ): List[TableRegion] = {
   ...
   */
  private def mergeWithFormulaRegions(
    tableRegions: List[TableRegion],
    formulaRegions: List[TableRegion]
  ): List[TableRegion] = {
    ...
    for (formulaRegion <- formulaRegions) {
      ...
      if (overlappingTables.isEmpty) {
        ...
        val shouldMerge = overlappingTables.exists { table =>
          val overlapArea =
            table.width.min(formulaRegion.width) * table.height.min(formulaRegion.height)
        ...
        if (shouldMerge) {
          ...
          val mergedRegion =
            TableRegion(minRow, maxRow, minCol, maxCol, allAnchorRows, allAnchorCols)
  ...
  /**
   * Implementation of SplittedEmptyLinesFilter from original spec. Detects continuous empty
   * rows/cols that can split a box into unrelated regions.
   */
  def applySplitEmptyLinesFilter(
    grid: SheetGrid,
    tableRegions: List[TableRegion]
  ): List[TableRegion] = {
  ...
  private def verifyBoxSplit(grid: SheetGrid, region: TableRegion): Boolean = {
    ...
    for (i <- region.topRow + 3 + rowOffset until region.bottomRow - 4) {
      ...
      val emptyRows = (i to i + 2).forall(row =>
    ...
    for (j <- region.leftCol + 3 + colOffset until region.rightCol - 4) {
      ...
      val emptyCols = (j to j + 2).forall(col =>
