 ...
 *
 * This is a marker trait that extends Transform and provides a simpler API
 * for implementations that always produce exactly one output for each input.
 *
 * Examples:
...
trait Conversion[I <: Mime, O <: Mime] extends Transform[I, O]:
   ...
   * @return A ZIO effect producing the converted content
   */
  def convert(input: Content[I]): ZIO[Any, TransformError, Content[O]]

  /**
   * Final implementation of Transform.apply that delegates to convert.
   * Always returns a Chunk of exactly one element.
   */
...
object Conversion:
  ...
  /**
   * Identity conversion that passes content through unchanged.
   * Useful as a starting point for pipelines.
   */
