...
/**
 * A Splitter is a 1:N Transform that splits content into multiple fragments
 * of the same output MIME type.
 *
 * This is used when the output MIME type is known at compile time and is
 ...
 * @tparam O Output MIME type (same for all fragments)
 */
trait Splitter[I <: Mime, O <: Mime] extends Transform[I, O]:
   ...
   * @return A ZIO effect producing a Chunk of fragments
   */
  def split(input: Content[I]): ZIO[Any, TransformError, Chunk[Fragment[O]]]

  /**
   * Implementation of Transform.apply that delegates to split.
   * Extracts the content from each fragment.
   */
   ...
   *
   * Default implementation materializes all fragments first.
   * Override for true streaming where fragments can be produced incrementally.
   */
...
/**
 * A DynamicSplitter is a 1:N Transform that splits content into multiple fragments
 * where each fragment can have a different MIME type determined at runtime.
 *
...
trait DynamicSplitter[I <: Mime] extends Transform[I, Mime]:
   ...
   *
   * @param input The input content to split
   * @return A ZIO effect producing a Chunk of dynamic fragments
   */
  def splitDynamic(input: Content[I]): ZIO[Any, TransformError, Chunk[DynamicFragment]]

  /**
   * Implementation of Transform.apply that delegates to splitDynamic.
   * Extracts the content from each fragment.
   */
...
object DynamicSplitter:
  ...
  def apply[I <: Mime](
    ...
    new DynamicSplitter[I]:
      override def splitDynamic(input: Content[I]): ZIO[Any, TransformError, Chunk[DynamicFragment]] =
        f(input)
