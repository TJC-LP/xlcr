...
object TransformMacros:
  ...
  /**
   * Require a direct conversion exists at compile time. Compilation fails if no `given
   * Conversion[I, O]` is in scope.
   */
  inline def requireConversion[I <: Mime, O <: Mime](using c: Conversion[I, O]): Conversion[I, O] =
    c

  /**
   * Require a splitter exists at compile time. Compilation fails if no `given Splitter[I, O]` is in
   * scope.
   */
  inline def requireSplitter[I <: Mime, O <: Mime](using s: Splitter[I, O]): Splitter[I, O] = s

  /**
   * Require a transform path exists at compile time (direct or composed). Uses CanTransform to find
   * paths through intermediate types.
   */
  ...
  private def findConversionImpl[I <: Mime: Type, O <: Mime: Type](using
    Quotes
  ): Expr[Option[Conversion[I, O]]] =
  ...
  private def findSplitterImpl[I <: Mime: Type, O <: Mime: Type](using
    Quotes
  ): Expr[Option[Splitter[I, O]]] =
    import quotes.reflect.*
