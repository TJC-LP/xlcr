  ...
  private object PngRenderer extends Renderer[M, ImagePng.type] {
    override def render(model: M): FileContent[ImagePng.type] = {
      Try {
        ...
        case Failure(e) => 
          throw RendererError(s"Failed to render PDF as PNG: ${e.getMessage}", Some(e))
      }
    }
  ...
  private object JpegRenderer extends Renderer[M, ImageJpeg.type] {
    override def render(model: M): FileContent[ImageJpeg.type] = {
      Try {
        ...
        case Failure(e) => 
          throw RendererError(s"Failed to render PDF as JPEG: ${e.getMessage}", Some(e))
      }
    }
    ...
    private def renderPdfPage(pdfBytes: Array[Byte], pageIndex: Int, dpi: Int, quality: Float): Array[Byte] = {
      ...
      try {
        ...
        params.setCompressionQuality(quality) // Between 0 and 1, where 1 is highest quality
...
object PdfImageRenderer {
   ...
   */
  def renderPageAsPng(pdfBytes: Array[Byte], pageIndex: Int, config: RenderConfig): Array[Byte] = {
   ...
   */
  def renderPageAsJpeg(pdfBytes: Array[Byte], pageIndex: Int, config: RenderConfig): Array[Byte] = {
  ...
  private def renderPageAsImage(pdfBytes: Array[Byte], pageIndex: Int, format: String, config: RenderConfig): Array[Byte] = {
    ...
    try {
      ...
      var image = renderer.renderImageWithDPI(pageIndex, config.dpi, ImageType.RGB)
      
      // Scale down if dimensions exceed max
      image = scaleImageIfNeeded(image, config.maxWidthPixels, config.maxHeightPixels)
   ...
   */
  private def scaleImageIfNeeded(image: BufferedImage, maxWidth: Int, maxHeight: Int): BufferedImage = {
   ...
   */
  private def renderJpeg(image: BufferedImage, initialQuality: Float, maxSizeBytes: Long): Array[Byte] = {
    var quality = initialQuality
    var imageBytes: Array[Byte] = null
    var attempts = 0
    
    // Try up to 5 times with decreasing quality if size exceeds limit
    while ((imageBytes == null || imageBytes.length > maxSizeBytes) && attempts < 5) {
      val baos = new ByteArrayOutputStream()
      val outputStream = new MemoryCacheImageOutputStream(baos)
      ...
      if (imageBytes.length > maxSizeBytes && attempts < 4) {
        quality *= 0.8f // Reduce quality by 20% each time
      }
   ...
   */
  private def renderPng(image: BufferedImage, maxSizeBytes: Long): Array[Byte] = {
    var currentImage = image
    var imageBytes: Array[Byte] = null
    var attempts = 0
    
    // Try up to 3 times with decreasing size if size exceeds limit
    while ((imageBytes == null || imageBytes.length > maxSizeBytes) && attempts < 3) {
      val baos = new ByteArrayOutputStream()
