...
class MimeType private (
   ...
   */
  def getParameter(name: String): Option[String] = parameters.get(name.toLowerCase)
   ...
   */
  def withDelimiter(delimiter: String): MimeType = withParameter("delimiter", delimiter)
   ...
   */
  def isType(baseType: String): Boolean = this.baseType.equalsIgnoreCase(baseType)
...
object MimeType extends Serializable {
  ...
  def apply(baseType: String, subType: String, parameters: Map[String, String] = Map.empty): MimeType =
    new MimeType(baseType.toLowerCase, subType.toLowerCase, parameters.map { case (k, v) => k.toLowerCase -> v })
  ...
  def parse(mimeTypeStr: String): MimeType = {
    mimeTypeStr match {
      ...
      case MimeTypeRegex(baseType, subType, paramStr) =>
        ...
        val params = ParamRegex.findAllMatchIn(paramStr + ";").map { m =>
  ...
  val ApplicationVndMsOutlook: MimeType = MimeType("application", "vnd.ms-outlook")
  ...
  val ApplicationVndMsPowerpoint: MimeType = MimeType("application", "vnd.ms-powerpoint")
  ...
  val values: Seq[MimeType] = Seq(
    TextPlain, TextHtml, TextMarkdown, TextCsv, TextXml, TextCss, TextJavascript,
    ...
    ApplicationJavascript, ApplicationXhtml,
    ImageJpeg, ImagePng, ImageGif, ImageBmp, ImageSvgXml, ImageTiff, ImageWebp,
    MessageRfc822,
