...
/**
 * Base exception for all document splitting failures. Provides rich context about the failure
 * including MIME type, strategy, and custom metadata.
 *
 * @param message
 *   The error message
 * @param cause
 ...
 * @param mimeType
 *   The MIME type of the document that failed to split
 * @param strategy
 *   The splitting strategy that was attempted (optional)
 * @param context
...
class SplitException(
   ...
   *   The context value
   * @return
   *   A new SplitException with the added context
   */
  ...
  /**
   * Override getMessage to include rich context information. This ensures that logs and error
   * reports contain all relevant details.
   */
  override def getMessage: String = {
    val contextStr =
      if (context.isEmpty) "" else s" [${context.map { case (k, v) => s"$k=$v" }.mkString(", ")}]"
...
/**
 * Thrown when the requested splitting strategy doesn't match the document type. For example,
 * requesting page splitting on an Excel file.
 *
 * @param mimeType
...
class InvalidStrategyException(
   ...
   *
   * @param validStrategies
   *   List of strategies that would work for this MIME type
   * @return
   *   A new exception with suggestions in the context
   */
...
/**
 * Thrown when a document appears to be corrupted or malformed. This helps distinguish between
 * configuration errors and actual document problems.
 *
 * @param mimeType
 *   The MIME type of the document
 * @param details
 *   Specific details about the corruption
 * @param cause
 *   The underlying cause (often from the parsing library)
 */
...
/**
 * Thrown when a document is empty or has no content to split. Common for emails with no
 * body/attachments or blank PDFs.
 *
 * @param mimeType
 *   The MIME type of the document
 * @param details
 *   Additional details about what was expected
 */
...
class SplitTimeoutException(
  ...
  details: String = ""
      ...
      s"Splitting operation timed out after ${timeoutMs}ms${if (details.nonEmpty) s": $details"
        else ""}",
      mimeType = mimeType
    ) {
...
/**
 * Thrown when document exceeds configured size limits. Helps prevent memory issues and zipbomb
 * attacks.
 *
 * @param mimeType
 *   The MIME type of the document
 * @param actualSize
 *   The actual size that exceeded limits
 * @param maxSize
 *   The configured maximum size
 * @param sizeType
 *   What was measured (e.g., "bytes", "chunks", "files")
 */
