 ...
 * Includes support for failure tracking and custom metadata.
 */
case class DocChunk[T <: MimeType](
  ...
  /**
   * Alias for attrs to provide a more intuitive name.
   * New code should use metadata instead of attrs.
   */
  def metadata: Map[String, String] = attrs
  
  /**
   * Check if this chunk represents a splitting failure.
   * A chunk is considered failed if it has a "split_status" of "failed".
   * 
   * @return true if this chunk represents a failure
   */
   ...
   * 
   * @return Some(error message) if this is a failed chunk, None otherwise
   */
   ...
   * 
   * @return Some(error type) if this is a failed chunk, None otherwise
   */
   ...
   * @return A new DocChunk with the added metadata
   */
   ...
   * Sets standard metadata fields for error tracking.
   * 
   * @param error The error message
   * @param errorType The type of error (e.g., exception class name)
   * @return A new DocChunk marked as failed
   */
  ...
  /**
   * Mark this chunk as successfully split.
   * Useful when using TagAndPreserve mode to explicitly mark successes.
   * 
   * @return A new DocChunk marked as successful
   */
