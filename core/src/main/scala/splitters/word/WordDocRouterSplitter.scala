...
/**
 * Router splitter for DOC files that dispatches to the appropriate implementation based on the
 * splitting strategy.
 */
object WordDocRouterSplitter extends DocumentSplitter[MimeType.ApplicationMsWord.type]
  ...
  override def split(
    ...
    strategy match {
      case Some(SplitStrategy.Page) =>
        logger.debug("Routing DOC to page splitter")
        WordDocPageSplitter.split(content, cfg.copy(strategy = Some(SplitStrategy.Page)))

      case Some(SplitStrategy.Heading) =>
        logger.debug("Routing DOC to heading splitter")
        WordDocHeadingSplitter.split(content, cfg.copy(strategy = Some(SplitStrategy.Heading)))
