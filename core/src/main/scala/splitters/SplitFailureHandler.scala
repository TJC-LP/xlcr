...
/**
 * Trait that provides failure handling capabilities for document splitters. Mix this trait into
 * DocumentSplitter implementations to get consistent failure handling behavior based on the
 * configured SplitFailureMode.
 */
trait SplitFailureHandler {
  ...
  /**
   * Handle a splitting failure according to the configured failure mode. This method implements the
   * core logic for each failure mode type.
   *
   * @param mode
   *   The failure mode to use
   * @param content
   *   The document content that failed to split
   * @param error
   *   The error message
   * @param cause
   *   The underlying exception (optional)
   * @param context
   *   Additional context about the failure
   * @tparam T
   *   The MIME type of the content
   * @return
   *   A sequence of chunks based on the failure mode
   */
  protected def handleSplitFailure[T <: MimeType](
    ...
    case SplitFailureMode.PreserveAsChunk =>
      logger.warn(
        s"Split failed for ${content.mimeType.mimeType}, preserving as single chunk: $error"
      )
  ...
  /**
   * Wrap splitting logic with failure handling. This method should be used to wrap the main
   * splitting logic in DocumentSplitter implementations. It automatically handles exceptions based
   * on the configured failure mode.
   *
   * @param content
   *   The document content to split
   * @param config
   *   The split configuration including failure mode
   * @param splitLogic
   *   The actual splitting logic to execute
   * @tparam T
   *   The MIME type of the content
   * @return
   *   A sequence of chunks, with failures handled according to the mode
   */
  ...
  /**
   * Helper method to handle invalid strategy errors. Provides consistent error handling when a
   * splitter doesn't support the requested strategy.
   *
   * @param content
   ...
   *   List of strategies this splitter supports
   * @tparam T
   *   The MIME type of the content
   * @return
   *   A sequence of chunks based on the failure mode
  ...
  /**
   * Helper method to handle empty document errors. Provides consistent handling when a document has
   * no content to split.
   *
   * @param content
   *   The document content
