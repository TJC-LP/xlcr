...
object PipelineStep {
  ...
  def apply[A, B](f: A => B): PipelineStep[A, B] = new PipelineStep[A, B] {
    override def run(input: A): B = f(input)
  }
  ...
  implicit final class StepOps[A, B](private val self: PipelineStep[A, B]) extends AnyVal {
     ...
     */
    def fanOut[C, D](left: PipelineStep[B, C], right: PipelineStep[B, D]): PipelineStep[A, (C, D)] =
      (a: A) => {
        val b = self.run(a)
    ...
    def split[C, D](left: PipelineStep[B, C], right: PipelineStep[B, D]): PipelineStep[A, (C, D)] =
      fanOut(left, right)
  }
}
