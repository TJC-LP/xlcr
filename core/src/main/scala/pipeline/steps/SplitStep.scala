...
/**
 * Split a document into multiple smaller `FileContent` chunks (pages, sheets, slides, …) using the
 * generic [[DocumentSplitter]] registry.
 *
 * Autodetection: if no explicit [[SplitStrategy]] was supplied we will choose an appropriate one
 * based on the *input* MIME type so that the caller does not have to worry about the concrete
 * document family.
 */
