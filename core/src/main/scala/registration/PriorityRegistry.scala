...
import types.Prioritized

/**
 * A thread-safe registry that stores values with priorities. For each key, the registry maintains a
 * sorted list of values by descending priority.
 *
 * @tparam K
 *   The type of keys in the registry
 * @tparam V
 *   The type of values in the registry
 * @param isSubtype
...
class PriorityRegistry[K, V <: Prioritized](
  ...
  /**
   * Registers a value for a key with the given priority. If a value with the same identity is
   * already registered for the key, it will be replaced with the new priority.
   *
   * @param key
   *   The key to register the value for
   * @param value
   ...
   * @return
   *   Some(value) if a value is registered for the key, None otherwise
   */
   ...
   *   The key to get the value for
   * @return
   *   Some(value) if a value is registered for the key or any of its supertypes, None otherwise
   */
   ...
   * @return
   *   List of registered values, sorted by priority
   */
   ...
   *   The key to check
   * @return
   *   true if at least one value is registered for the key, false otherwise
   */
   ...
   *
   * @return
   *   Set of all registered values
   */
  def values: Set[V] =
    registry.values.flatMap(_.toSet).toSet

  /**
   * Gets all entries in the registry as key-value pairs. For each key, the highest priority value
   * is returned.
   *
   * @return
   *   Map of keys to their highest priority values
   */
  def entries: Map[K, V] =
