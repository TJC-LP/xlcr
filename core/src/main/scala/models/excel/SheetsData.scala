...
/**
 * SheetsData is a container for multiple SheetData instances. It implements the Model trait so it
 * can load (fromMimeType) or export (toMimeType) data as JSON, Excel, or Markdown.
 */
final case class SheetsData(sheets: List[SheetData]) extends Model with Mergeable[SheetsData] {

  /**
   * Merge logic for SheetsData. Merges each sheet by name. If the "other" SheetsData has a sheet of
   * the same name, merge them. Otherwise, add it.
   */
