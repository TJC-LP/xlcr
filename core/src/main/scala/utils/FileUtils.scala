...
object FileUtils {
   ...
   *
   * @param path
   *   The Path where data should be written.
   * @param data
  ...
  /**
   * Execute a function with a temporary file, ensuring the file is deleted afterward, regardless of
   * success or failure.
   *
   * @param prefix
   *   Prefix string to be used in generating the file's name.
   * @param suffix
   *   Suffix string to be used in generating the file's name (e.g. ".xlsx").
   * @param f
   *   A function that takes a temporary Path and returns a T.
   * @tparam T
   ...
   * @return
   *   A Try containing the JSON string, or a Failure with the exception.
   */
   ...
   * @return
   *   A Try containing the file's bytes, or a Failure with the exception.
   */
  ...
  /**
   * Write a JSON string to a file with UTF-8 encoding. Creates parent directories if they don't
   * exist.
   *
   * @param path
   *   The Path where JSON should be written.
   * @param jsonContent
