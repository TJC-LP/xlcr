...
import jakarta.mail.{BodyPart, Message, Multipart, Part, Session}
...
/**
  ...
  */
class EmailAttachmentSplitter extends DocumentSplitter[MimeType.MessageRfc822.type] {
    ...
    def harvest(part: Part): Unit = {
      if (part.isMimeType("multipart/*")) {
        ...
        if (Part.ATTACHMENT.equalsIgnoreCase(disposition) || disposition == "inline" && ctype.startsWith("image/")) {
          ...
          val bytes = part.getInputStream.readAllBytes()
          val mime  = if (part.isMimeType("text/html")) MimeType.TextHtml else MimeType.TextPlain
