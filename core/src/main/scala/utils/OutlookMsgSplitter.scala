...
class OutlookMsgSplitter
  ...
  override def split(
    ...
    val chunks = scala.collection.mutable.ListBuffer.empty[DocChunk[_ <: MimeType]]
    if (hasBody) {
      val mime = if (msg.getHtmlBody != null) MimeType.TextHtml else MimeType.TextPlain
      val subj = Option(msg.getSubject).getOrElse("body")
      chunks += DocChunk(FileContent(bodyText.getBytes("UTF-8"), mime), subj, 0, totalChunks)
    }
    atts.zipWithIndex.foreach { case (att, idx0) =>
      val idx = if (hasBody) idx0 + 1 else idx0
      val name = Option(att.getAttachLongFileName).map(_.toString).filter(_.nonEmpty)
