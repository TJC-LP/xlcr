...
class ArchiveEntrySplitter extends DocumentSplitter[MimeType] {
  ...
  private def trackExtractedSize(sessionId: java.util.UUID, byteCount: Long, maxSize: Long): Boolean = {
    ...
    if (newTotal > maxSize) {
      logger.warn(s"Archive extraction aborted: size limit exceeded (${newTotal} > ${maxSize} bytes)")
      false
    } else {
      true
    }
  ...
  override def split(
    ...
    try {
      ...
      val chunks = content.mimeType match {
        ...
        case MimeType.ApplicationGzip => splitSingleEntryCompressed(content, cfg, CompressorStreamFactory.GZIP, sessionId)
        case MimeType.ApplicationBzip2 => splitSingleEntryCompressed(content, cfg, CompressorStreamFactory.BZIP2, sessionId)
        case MimeType.ApplicationXz => splitSingleEntryCompressed(content, cfg, CompressorStreamFactory.XZ, sessionId)
        case MimeType.ApplicationSevenz => splitZip(content, cfg, sessionId) // Fallback to zip handling for 7z
  ...
  private def splitZip(
    ...
    try {
      ...
      while (entry != null) {
        ...
        if (entryName.startsWith("__MACOSX") || entryName.contains("/__MACOSX/")) {
          ...
          if (size > 0) {
            ...
            val wouldExceedLimit = !trackExtractedSize(sessionId, size, cfg.maxTotalSize)
            if (wouldExceedLimit) {
              logger.warn(s"Skipping ZIP entry ${entry.getName}: would exceed size limit")
              zipInput.closeEntry()
              entry = zipInput.getNextEntry()
              // Skip to next entry
          ...
          while ({ len = zipInput.read(buffer); len > 0 }) {
            ...
            if (bytesRead > maxEntrySize) {
              logger.warn(s"ZIP entry ${entryName} exceeded max size (${bytesRead} > ${maxEntrySize})")
              scala.util.control.Breaks.break()
            }
          ...
          if (size <= 0 && !trackExtractedSize(sessionId, entryData.length, cfg.maxTotalSize)) {
          ...
          if (compressedSize > 0 && entryData.length / compressedSize > 1000) {
            logger.warn(s"Suspicious compression ratio for ${entryName}: ${entryData.length / compressedSize}")
          }
      ...
      if (entriesProcessed > 0 && potentialCompressedSize > 0 && totalUncompressedSize / potentialCompressedSize > 100) {
        logger.warn(s"Potential ZIP bomb detected: compression ratio ${totalUncompressedSize / potentialCompressedSize}")
        logger.warn(s"Archive contains ${entriesProcessed} entries, uncompressed: ${totalUncompressedSize}")
      }
    } catch {
      case e: Exception =>
    ...
    try {
      ...
      breakable {
        ...
        if (totalBytesRead > maxDecompressedSize) {
          logger.warn(s"Compressed file exceeded max size limit (${totalBytesRead} > ${maxDecompressedSize})")
          scala.util.control.Breaks.break()
        }
      ...
      // Check compression ratio (zipbomb protection)
      val compressionRatio = decompressedBytes.length.toDouble / content.data.length
      ...
      case e: Exception =>
        ...
        logger.error(s"Error decompressing ${compressorName}: ${e.getMessage}", e)
