...
/**
 * Performance and edge case tests for TextSplitter.
 * These tests specifically target issues that were fixed:
 * 1. High overlap causing O(n²) performance
 * 2. Incorrect paragraph position tracking with duplicates
...
First paragraph"""
    ...
    chunks.foreach { chunk =>
      ...
      if (matches.isDefined) {
        ...
        if (!contentMatches) {
          println(s"Position mismatch: expected at $start-$end: '$expectedContent', got: '$actualContent'")
...
Another small paragraph"""
  ...
  "TextSplitter strategy handling" should "use character-based splitting for Chunk strategy" in {
    val content = FileContent("Simple text content for testing chunks".getBytes(), MimeType.TextPlain)
      .asInstanceOf[FileContent[MimeType.TextPlain.type]]
