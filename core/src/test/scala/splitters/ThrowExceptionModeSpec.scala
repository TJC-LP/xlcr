...
/**
 * Comprehensive test suite for ThrowException failure mode.
 * Tests that all splitters throw the correct exception types with proper context.
 */
class ThrowExceptionModeSpec extends AnyWordSpec with Matchers {
  ...
  "ArchiveEntrySplitter in ThrowException mode" should {
    ...
    "throw CorruptedDocumentException for invalid ZIP" in {
      ...
      val corruptedZip = Array[Byte](0x50, 0x4B, 0x03, 0x04, 0xFF.toByte, 0xFF.toByte) // Valid ZIP header then garbage
