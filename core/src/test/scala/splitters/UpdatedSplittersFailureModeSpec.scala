...
class UpdatedSplittersFailureModeSpec extends AnyFlatSpec with Matchers {
  ...
  "All updated splitters" should "maintain backward compatibility" in {
    ...
    noException should be thrownBy ZipEntrySplitter.split(zipContent, defaultConfig.copy(strategy = Some(SplitStrategy.Embedded)))
    
    val emailContent = FileContent(invalidEmailBytes, MimeType.MessageRfc822)
    noException should be thrownBy EmailAttachmentSplitter.split(emailContent, defaultConfig.copy(strategy = Some(SplitStrategy.Attachment)))
  }
}