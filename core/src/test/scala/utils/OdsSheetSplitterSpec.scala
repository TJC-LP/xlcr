...
class OdsSheetSplitterSpec extends AnyFlatSpec with Matchers {
  ...
  "OdsSheetSplitter" should "handle empty or null input" in {
    ...
    val result = splitter.split(emptyContent, SplitConfig(Some(SplitStrategy.Sheet)))
  ...
  it should "split an ODS file into individual sheets" in {
    ...
    try {
      ...
      // Split the ODS file
      val result = splitter.split(content, SplitConfig(Some(SplitStrategy.Sheet)))
      ...
      for (i <- 0 until 3) {
        ...
        val doc = OdfSpreadsheetDocument.loadDocument(new ByteArrayInputStream(chunk.content.data))
