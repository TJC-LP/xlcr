...
/**
 * Helper utilities for testing failure modes across different splitters. Provides common test data,
...
object FailureModeTestHelper extends Matchers {
  ...
  def largeBytes(sizeMB: Int): Array[Byte] =
    Array.fill(sizeMB * 1024 * 1024)(0x41.toByte) // 'A' repeated
  ...
  trait CommonFailureModeTests { self: Matchers =>
    ...
    def testInvalidStrategy[T <: MimeType](
      ...
      thrown.getMessage should include(
        s"Strategy '${invalidStrategy.displayName}' is not supported"
      )
