...
class WordHeadingSplitterSpec extends AnyFlatSpec with Matchers {
  ...
  it should "split a docx by Heading1 paragraphs" in {
    val docBytes: Array[Byte] = {
      ...
      val h1 = doc.createParagraph(); h1.setStyle("Heading1");
      h1.createRun().setText("Intro")
      val p1 = doc.createParagraph(); p1.createRun().setText("Lorem ipsum 1")

      val h2 = doc.createParagraph(); h2.setStyle("Heading1");
      h2.createRun().setText("Details")
    ...
    ) shouldBe MimeType.ApplicationVndOpenXmlFormatsWordprocessingmlDocument
  }
}
