...
class AsposeSplittersFailureModeSpec extends AnyFlatSpec with Matchers {
  ...
  it should "handle malformed emails" in {
    ...
    val configs = Seq(
      ("ThrowException", SplitConfig(
        strategy = Some(SplitStrategy.Attachment),
        failureMode = SplitFailureMode.ThrowException
      )),
      ("PreserveAsChunk", SplitConfig(
        strategy = Some(SplitStrategy.Attachment),
        failureMode = SplitFailureMode.PreserveAsChunk
      )),
      ("DropDocument", SplitConfig(
        strategy = Some(SplitStrategy.Attachment),
        failureMode = SplitFailureMode.DropDocument
      )),
      ("TagAndPreserve", SplitConfig(
        strategy = Some(SplitStrategy.Attachment),
        failureMode = SplitFailureMode.TagAndPreserve
      ))
    )
  ...
  "All Aspose splitters" should "maintain backward compatibility" in {
    ...
    noException should be thrownBy PdfPageAsposeSplitter.split(pdfContent, defaultConfig.copy(strategy = Some(SplitStrategy.Page)))
    
    val zipContent = FileContent(invalidZipBytes, MimeType.ApplicationZip)
    noException should be thrownBy ZipArchiveAsposeSplitter.split(zipContent, defaultConfig.copy(strategy = Some(SplitStrategy.Embedded)))
    
    val emailContent = FileContent(invalidEmailBytes, MimeType.MessageRfc822)
    noException should be thrownBy EmailAttachmentAsposeSplitter.split(emailContent, defaultConfig.copy(strategy = Some(SplitStrategy.Attachment)))
    
    val excelContent = FileContent(invalidExcelBytes, MimeType.ApplicationVndOpenXmlFormatsSpreadsheetmlSheet)
    noException should be thrownBy ExcelXlsxSheetAsposeSplitter.split(excelContent, defaultConfig.copy(strategy = Some(SplitStrategy.Sheet)))
  }
}