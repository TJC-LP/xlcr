...
object PowerPointToPdfAsposeBridge
  ...
  override private[bridges] def inputParser
      : Parser[ApplicationVndMsPowerpoint.type, M] =
  ...
  /** Parser that simply wraps the input bytes into a PptDocModel.
    */
  private object PowerPointToPdfAsposeParser
      extends Parser[ApplicationVndMsPowerpoint.type, M] {
  ...
  private object PowerPointToPdfAsposeRenderer
    ...
    override def render(model: M): FileContent[ApplicationPdf.type] = {
      try {
        ...
        logger.info(
          s"PowerPoint to PDF conversion successful, output size = ${pdfBytes.length} bytes."
        )
        FileContent[ApplicationPdf.type](pdfBytes, ApplicationPdf)
      } catch {
        case ex: Exception =>
          logger.error(
            "Error during PowerPoint to PDF conversion with Aspose.Slides.",
            ex
          )
          throw RendererError(
