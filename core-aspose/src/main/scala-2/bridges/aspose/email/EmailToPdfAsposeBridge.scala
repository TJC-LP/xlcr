...
object EmailToPdfAsposeBridge
    extends HighPrioritySimpleBridge[MessageRfc822.type, ApplicationPdf.type] {
  ...
  private object EmailToPdfAsposeParser extends Parser[MessageRfc822.type, M] {
    override def parse(input: FileContent[MessageRfc822.type]): M = {
      ...
      FileContent[MimeType.MessageRfc822.type](
        input.data,
  ...
  private object EmailToPdfAsposeRenderer
    ...
    override def render(model: M): FileContent[ApplicationPdf.type] = {
      try {
        AsposeLicense.initializeIfNeeded()
        logger.info(
          "Rendering EmailDocModel to PDF using Aspose.Email + Aspose.Words."
        )
        ...
        logger.info(
          s"Email -> PDF conversion successful, output size = ${pdfBytes.length} bytes."
        )
