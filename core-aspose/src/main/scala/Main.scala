...
object Main {
  ...
  private val parser = {
    ...
    OParser.sequence(
      ...
      opt[Int]("max-recursion-depth")
        ...
        .text(
          "Maximum recursion depth for nested archives (default: 5). Used with --recursive."
        ),
  ...
  def main(args: Array[String]): Unit =
    OParser.parse(parser, args, AsposeConfig()) match {
      case Some(cfg) =>
        // Initialize Aspose licenses (runtime concern, still needed)
        applyLicenses(cfg)
        // The registries (BridgeRegistry, DocumentSplitter) will load Aspose components
        // via ServiceLoader if the core-aspose JAR is on the classpath.
    ...
    cfg.licenseTotal match {
      ...
      case None =>
        ...
        if (anyExplicit) {
          ...
          logger.info(
            "No explicit license paths provided – using env vars / auto‑discovery …"
          )
