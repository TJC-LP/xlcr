...
/**
 * Router splitter for DOCX files using Aspose that dispatches to the appropriate implementation
 * based on the splitting strategy. Has HIGH priority to override POI-based router.
 */
object WordDocxRouterAsposeSplitter extends DocumentSplitter[MimeType.ApplicationVndOpenXmlFormatsWordprocessingmlDocument.type] 
  ...
  override def split(
    ...
    strategy match {
      case Some(SplitStrategy.Page) =>
        logger.debug("Routing DOCX to Aspose page splitter")
        WordDocxPageAsposeSplitter.split(content, cfg.copy(strategy = Some(SplitStrategy.Page)))
        
      case Some(SplitStrategy.Heading) =>
        logger.debug("Routing DOCX to Aspose heading splitter")
        WordDocxHeadingAsposeSplitter.split(content, cfg.copy(strategy = Some(SplitStrategy.Heading)))
