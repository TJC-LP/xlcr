...
/**
 * AsposeConfig is a scopt config object for parsing CLI arguments, including paths to individual
 * Aspose product licenses or a total license.
 *
 * If no license paths are specified, the system will automatically try to detect license files
 * from:
