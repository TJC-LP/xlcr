...
trait ExcelToPdfLibreOfficeBridgeImpl[I <: MimeType]
  ...
  private[bridges] def outputRenderer: Renderer[M, ApplicationPdf.type] =
    ExcelToPdfLibreOfficeRenderer

  /**
   * Renderer that uses LibreOffice (via JODConverter) to convert Excel -> PDF.
   */
  private object ExcelToPdfLibreOfficeRenderer
      extends SimpleRenderer[M, ApplicationPdf.type] {
    override def render(model: M): FileContent[ApplicationPdf.type] =
      try {
        logger.info("Rendering Excel spreadsheet to PDF using LibreOffice.")

        val pdfBytes = convertExcelToPdf(model.data)

        logger.info(
          s"Successfully converted Excel to PDF using LibreOffice, output size = ${pdfBytes.length} bytes."
        )
        FileContent[ApplicationPdf.type](pdfBytes, ApplicationPdf)
      } catch {
        case ex: Exception =>
          logger.error(
            "Error during Excel -> PDF conversion with LibreOffice.",
            ex
          )
          throw RendererError(
  ...
  private def convertExcelToPdf(excelBytes: Array[Byte]): Array[Byte] = {
    ...
    try {
      // Write input Excel document to temp file
      Files.write(inputFile.toPath, excelBytes)
      logger.debug(s"Wrote Excel input to temp file: ${inputFile.getAbsolutePath}")
