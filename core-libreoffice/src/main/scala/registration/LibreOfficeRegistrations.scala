...
import bridges.excel.{ ExcelXlsToPdfLibreOfficeBridge, ExcelXlsmToPdfLibreOfficeBridge, ExcelXlsxToPdfLibreOfficeBridge }
import bridges.odf.OdsToPdfLibreOfficeBridge
import bridges.powerpoint.{ PowerPointPptToPdfLibreOfficeBridge, PowerPointPptxToPdfLibreOfficeBridge }
import bridges.word.{ WordDocToPdfLibreOfficeBridge, WordDocxToPdfLibreOfficeBridge }
import spi.{ BridgeInfo, BridgeProvider }
import types.MimeType

/**
 * Provides the LibreOffice bridges for registration via ServiceLoader.
 * These have DEFAULT priority and will act as fallbacks when Aspose (HIGH priority) is not available.
 *
 ...
 *
 * All bridges use LibreOffice via JODConverter for document conversions.
 * LibreOffice must be installed on the system for these bridges to work.
 */
