...
/**
 * Utilities for programmatically generating test documents. Creates minimal valid documents for
 * testing without file system dependencies.
 */
object DocumentGenerators {

  /**
   * Create a minimal valid XLSX (Excel 2007+) workbook. Contains one sheet with a single cell
   * containing "Test".
   */
  ...
  /**
   * Create a minimal valid XLSX with multiple cells and formulas. More realistic test document with
   * some complexity.
   */
  ...
  /**
   * Create a minimal valid DOCX (Word 2007+) document. Contains a single paragraph with text.
   */
  ...
  /**
   * Create a DOCX document with multiple paragraphs and formatting. More realistic test document.
   */
  ...
  /**
   * Create a minimal valid PPTX (PowerPoint 2007+) presentation. Contains one slide with a title.
   */
  ...
  /**
   * Create a PPTX presentation with multiple slides. More realistic test document.
   */
  ...
  /**
   * Create a minimal valid ODS (OpenDocument Spreadsheet) file. Uses the same Apache POI approach
   * but with ODF format.
   *
   * Note: For simplicity, we create an XLSX and use it as ODS since LibreOffice can read both
   * formats. A true ODS would use ODFDOM library.
   */
  ...
  /**
   * Create an invalid document (corrupted data) for error testing. Just returns text that's not a
   * valid document.
   */
   ...
   *
   * @param format
   *   The format ("xlsx", "docx", "pptx", "ods")
   * @return
   *   Byte array containing the document
   */
